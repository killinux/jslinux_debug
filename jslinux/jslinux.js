...
function term_start()
{
    ...
    term = new Term(80, 80, term_handler);
    term.open();
}
...
function start2(ret)
{
	console.log("jslinux.js:start2");
    if (ret < 0)
        return;
    init_state.start_addr = 0x10000;//haohao
    pc.load_binary("linuxstart.bin", init_state.start_addr, start3);
}
function start3(ret)
{
	console.log("jslinux.js:start3");
    var block_list;
    if (ret < 0)
        return;
